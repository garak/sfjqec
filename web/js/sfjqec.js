$(document).ready(function() {
  ...
  var showProducts = function(results) {
    ...
    if ($ul.find('li').length < 1) {  // no product found
      var $li = $('<li>').append('No results');
      $ul.append($li);
    }
  }
  ...
  var cartAdd = function(result, link) {
    ...
    if ($ul.length == 0) {  // no items yet
      ...
      $('div#cart').append($(result.a_empty));
    }
    ...
    $li.append(result.name).append(' ');
    $li.append(result.price).append(' ');
